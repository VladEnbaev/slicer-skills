...
Use this as the Slicer-style wrapper for systematic debugging: proactive, concise, and checkpointed.
...
5. **Offer fix options**: use `slicer-checkpoints` when multiple fixes are plausible or risk differs.
6. **Plan bugfix slices**: use `slicer-planning` when the fix needs decomposition or staged review.
7. **Execute bugfix slice**: use `slicer-execution`; usually include a regression test unless there is a clear reason not to.
8. **Verify**: prove the failure is gone and note remaining risk.
